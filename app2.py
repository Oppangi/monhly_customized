...
class Settings:
    ...
    def _resolve_template_path(self) -> Path:
        ...
        template_path = Path(__file__).parent / 'templates' / 'monthly_template.pptx'
        if template_path.exists():
            return template_path
           
        # Try alternative paths
        alternative_paths = [
            Path('templates/monthly_template.pptx'),
            Path.cwd() / 'templates' / 'monthly_template.pptx',
            Path.home() / 'templates' / 'monthly_template.pptx'
        ]
        ...
        # If not found, create a blank presentation as fallback
        logger.warning("Template not found at any standard location. Using blank presentation.")
        return Path(__file__).parent / 'templates' / 'monthly_template.pptx'
...
class PresentationGenerator:
    ...
    def generate_presentation(self, content: str, company_name: str,
        ...
        try:
            # Create a blank presentation (don't use template to avoid placeholders)
            prs = Presentation()
